...
const TaskCard: React.FC<TaskCardProps> = ({ task, user, onApply, isRecommended = false }) => {
  const handleApply = () => {
    onApply(task.id);
  };

  const canApply = user && !user.isCompany && task.status === 'Open';
    ...
    <div className={`bg-base-200 rounded-lg shadow-lg p-6 flex flex-col justify-between border border-base-300 hover:border-brand-primary transition-all duration-300 transform hover:-translate-y-1 ${isRecommended ? 'border-2 border-brand-secondary' : ''}`}>
      ...
      <div className="flex justify-between items-center mt-4 pt-4 border-t border-base-300">
        ...
        {canApply ? (
            <button 
                onClick={handleApply}
                className="bg-brand-primary hover:bg-brand-secondary text-white font-bold py-2 px-4 rounded-lg transition-transform duration-200 hover:scale-105"
            >
              Apply
            </button>
        ) : (
            <span className={`px-3 py-1 text-xs font-bold rounded-full ${
                ...
                task.status === 'Completed' ? 'bg-gray-500/20 text-gray-400' : 'bg-blue-500/20 text-blue-300'
            }`}>
              {task.status}
            </span>
        )}
